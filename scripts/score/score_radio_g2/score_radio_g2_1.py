...
gals_obs = batch['inputs'][0*768:1*768]
psfs = batch['psf'][0*768:1*768]
tikhos = batch['inputs_tikho'][0*768:1*768]

# Initiate instance of score
# set the value of gamma
g2 = score(gamma=gamma,rip=False,verbose=False)
...
for obs, psf, tikho in zip(gals_obs,psfs,tikhos):
    ...
    #deconvolve
    g2.deconvolve(obs=obs,psf=psf,thresholds=thresholds,first_guess=tikho)
    sol_g2 += [g2.solution]
    if i%10 == 0:
        print(i)
    i += 1

filename = data_path + 'score_radio_tikho_g2_1'
np.save(filename,np.array(sol_g2))
