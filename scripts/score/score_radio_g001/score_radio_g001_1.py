...
g001 = score(gamma=gamma,rip=False,verbose=False)
...
for obs, psf in zip(gals_obs,psfs):
    ...
    #deconvolve
    g001.deconvolve(obs=obs,psf=psf,thresholds=thresholds)
    sol_g001 += [g001.solution]
    if i%10 == 0:
        print(i)
    i += 1

filename = data_path + 'score_radio_g001_1'
np.save(filename,np.array(sol_g001))
