...
g01 = score(gamma=gamma,rip=False,verbose=False)
...
for obs, psf in zip(gals_obs,psfs):
    ...
    #deconvolve
    g01.deconvolve(obs=obs,psf=psf,thresholds=thresholds)
    sol_g01 += [g01.solution]
    if i%10 == 0:
        print(i)
    i += 1

filename = data_path + 'score_radio_g01_4'
np.save(filename,np.array(sol_g01))
