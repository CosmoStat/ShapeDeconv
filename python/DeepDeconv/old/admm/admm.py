...
class ADMM(SetUp):
    def __init__(self, x, z, mu, obs, psf, sigma, dnn,
             ...
             opti_method='FISTA', convolve_mode='scipy_fft'):
        SetUp.__init__(self)
        ...
        self._hth = get_conv2d(psf, psf, mode=self._convolve_mode, transp=True)
        self._hty = get_conv2d(obs, psf, mode=self._convolve_mode, transp=True)
        
    
    def _x_gradient_descent(self, max_iter=1000, stop_criterion=1e-6):
        x_temp = np.copy(self._x_old)
        for i in range(max_iter):
            grad = get_conv2d(x_temp, self._hth, mode=self._convolve_mode, transp=False) - self._hty
    ...
    def _fista(self, max_iter=500):
        ...
        t_fista_old = 1
        for i in range(max_iter):
            grad = get_conv2d(z_fista_old, self._hth, mode=self._convolve_mode, transp=False) - self._hty
